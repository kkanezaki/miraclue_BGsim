{
  ifstream ifs("migdal_ibe_Xe.dat");
  TString str, dummy;
  int n, l;
  ...
  c->cd(2)->DrawFrame(0, 0, 7e1, 0.6e-6,"probability (q_{e}=511eV);E_{e} / keV;#frac{1 dp^{c}}{2#pidE_{e}} / keV^{-1}");
  for(int n=0; n<6; n++) for(int l=0; l<3; l++) if(g_index[n][l]>0) graph[n][l]->Draw("sameL");

  c->Print("draw_migdal_ibe_Xe.png");



  double prob_total[6][3] = {};
  for(int n=0; n<6; n++){
    for(int l=0; l<3; l++){
      if(g_index[n][l]>0){
	for(int i=0; i<g_index[n][l]-1; i++){
	  //	  cout<<" prob = "<<prob[1][0][i]<<endl;
	  prob_total[n][l] += 0.5*(prob[n][l][i]+prob[n][l][i+1])*(ene[n][l][i+1]-ene[n][l][i]);
	}
	cout<<" prob_total["<<n<<"]["<<l<<"]="<<prob_total[n][l]<<endl;
      }
    }
  }
