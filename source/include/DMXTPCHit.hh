...
class DMXTPCHit : public G4VHit{
public:
  DMXTPCHit();
  ~DMXTPCHit();
  DMXTPCHit(const DMXTPCHit&);
  const DMXTPCHit& operator=(const DMXTPCHit&);
  int operator==(const DMXTPCHit&) const;
...
typedef G4THitsCollection<DMXTPCHit> DMXTPCHitsCollection;
extern G4Allocator<DMXTPCHit> DMXTPCHitAllocator;

inline void* DMXTPCHit::operator new(size_t){
  void* aHit;
  aHit = (void*) DMXTPCHitAllocator.MallocSingle();
  return aHit;
}

inline void DMXTPCHit::operator delete(void* aHit){
  DMXTPCHitAllocator.FreeSingle((DMXTPCHit*) aHit);
}
